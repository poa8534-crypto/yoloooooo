...
function VentureScoutPage({ candidates }: { candidates: Candidate[] }) {
  ...
  const hunterBacked = useMemo(() => candidates.filter(item => item.proposal_id), [candidates])
  useEffect(() => { if (!hunterBacked.some(item => item.id === candidateId)) setCandidateId(hunterBacked[0]?.id || '') }, [hunterBacked, candidateId])
  const candidate = hunterBacked.find(item => item.id === candidateId)
  ...
  return <section className="workspace">
    ...
    <div className="scout-guard"><div><strong>Venture Scout audit guardrail</strong><span>Fail-closed enforced</span></div><p>The Scout only works on games Meta Hunter has proposed a concept for; nothing else reaches it, and the backend refuses it rather than trusting this list. <strong>Audit idea</strong> critiques one exact Hunter proposal. <strong>Analyze game</strong> works from that game's captured evidence instead of its concept. Both are scoped to a solo beginner and a three-day MVP, and neither can invent facts or override a deterministic decision.</p></div>
    <div className="agent-columns"><article className="data-panel"><div className="panel-heading"><div><span>Solo MVP configuration</span><h2>Scout operation</h2></div></div>{hunterBacked.length ? <><label>Operation<select value={operation} onChange={event => setOperation(event.target.value as ScoutOperation)}>
